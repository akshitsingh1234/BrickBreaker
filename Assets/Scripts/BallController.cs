...
public class BallController : MonoBehaviourPunCallbacks,IInRoomCallbacks
{
    ...
    void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        if (this.gameObject.tag == "ball")
        {
            if((collision.gameObject.name == "leftborder") && (collision.gameObject.tag == "paddle"))
            {
                if(currentVel1.x==0)
            ...
            if ((collision.gameObject.name == "rightborder") && (collision.gameObject.tag == "paddle"))
            {
                if (currentVel1.x == 0)
                    this.gameObject.GetComponent<Rigidbody2D>().velocity = new Vector2(currentVel1.x + 0.3f, currentVel1.y * -1);
            }
        ...
        {
            if ((collision.gameObject.name == "leftborder") && (collision.gameObject.tag == "paddle1"))
            {
                if (currentVel1.x == 0)
                    this.gameObject.GetComponent<Rigidbody2D>().velocity = new Vector2(currentVel1.x - 0.3f, currentVel1.y * -1);
            }
            ...
            if ((collision.gameObject.name == "rightborder") && (collision.gameObject.tag == "paddle1"))
            {
                if (currentVel1.x == 0)
                    this.gameObject.GetComponent<Rigidbody2D>().velocity = new Vector2(currentVel1.x - 0.3f, currentVel1.y * -1);
            }
